...
class Worker(threading.Thread):
    ...
    def __init__(self, q, canvas, *args, **kwargs):
        self.q = q
        self.canvas = canvas
    ...
    def run(self):
        while True:
            try:
                ...
                logger.debug(f'a1 {self.q} {self.q.qsize()}')
                if self.q.qsize() > 0:
                    continue
                self.canvas.draw_method()
                self.q.task_done()
                logger.debug(f'a7 {self.q} {self.q.qsize()}')
            except queue.Empty:
                pass
...
class CanvasGraph(tk.Canvas):
    def __init__(
        ...
        self.zoom_handler = Worker(self.zoom_queue, self).start()
    ...
    #     self.redraw_queue_method()  # redraw the image
    ...
    def redraw_queue_method(self):
        self.zoom_queue.put_nowait([self])
